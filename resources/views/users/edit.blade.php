...
<x-app-layout>
    <div class="my-0 p-5 max-w-4xl mx-auto rounded-lg shadow-lg">
        <div class="flex flex-col sm:flex-row justify-between items-center border-b pb-4">
            <h3 class="text-xl font-semibold text-gray-200">Edit Data Users</h3>
        ...
        <div>
            <form action="{{ route('users.update', $users->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                <div class="mt-4">
                    ...
                    <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" required
                        autocomplete="new-name" value="{{ old('name', $users->name) }}" />
                ...
                <div class="mt-4">
                    ...
                    <x-text-input id="email" class="block mt-1 w-full" type="email" name="email" required
                        autocomplete="new-email" value="{{ old('email', $users->email) }}" />
                ...
                <div class="mt-4">
                    ...
                    <select id="role" name="role"
                        ...
                        <option value="pegawai" {{ old('role', $users->role) == 'pegawai' ? 'selected' : '' }}>Pegawai
                        </option>
                    </select>
